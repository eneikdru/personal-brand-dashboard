# 11. Risks, Forecast and Facts: Feature Adoption Rate

## Слабые стороны с точки зрения аналитической философии
- Adoption может измерять видимость функции, а не ее ценность.
- Высокий adoption иногда вызван принудительным UX или отсутствием альтернатив.
- Низкий adoption может быть нормальным для редкой, но критически важной функции.
- Метрика не отвечает, улучшила ли функция retention, revenue или North Star.

## Где метрика может обмануть
Метрика обманывает, когда ее начинают воспринимать как реальность, а не как модель реальности. Для `Feature Adoption Rate` главный риск - забыть, что показатель зависит от определения события, выбранного знаменателя, периода, сегмента и качества данных.
...
```text
Forecast Feature Users = eligible_users * expected_feature_adoption_rate

Risk Coefficient = 1 - Probability_of_Risk * Impact_of_Risk

Conservative Forecast = Base Forecast * K_total
K_total = `K_eligibility_accuracy` * `K_discoverability` * `K_value_repeatability` * `K_rollout_execution`
```
...
|---|---:|---|
| `K_eligibility_accuracy` | 0.70-1.00 | понижение, если eligible audience определена грубо |
| `K_discoverability` | 0.65-0.95 | понижение за риск, что пользователи функцию не увидят |
| `K_value_repeatability` | 0.60-0.95 | понижение, если нет доказанного repeat usage |
| `K_rollout_execution` | 0.75-0.98 | понижение за phased rollout, баги и permission issues |
...
- Фиксировать exposure, first_use, repeat_use и success_after_feature отдельно.
- Хранить eligible_user flag и reason.
- Записывать feature_version, rollout_group, experiment_id.
- Отдельно фиксировать abandonment inside feature flow.
