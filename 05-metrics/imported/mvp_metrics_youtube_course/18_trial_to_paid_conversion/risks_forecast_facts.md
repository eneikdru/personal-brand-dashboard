# 18. Risks, Forecast and Facts: Trial-to-Paid Conversion

## Слабые стороны с точки зрения аналитической философии
- Trial-to-paid может улучшиться за счет агрессивного auto-renew, ухудшая доверие и first renewal.
- Метрика часто смешивает trial starts, trial completions и billing attempts.
- Первая оплата не доказывает, что пользователь получил устойчивую ценность.
- Скидки и paywall pressure могут поднять conversion, но снизить LTV.

## Где метрика может обмануть
Метрика обманывает, когда ее начинают воспринимать как реальность, а не как модель реальности. Для `Trial-to-Paid Conversion` главный риск - забыть, что показатель зависит от определения события, выбранного знаменателя, периода, сегмента и качества данных.
...
```text
Forecast Paid Users = trial_starts * expected_trial_to_paid_rate

Risk Coefficient = 1 - Probability_of_Risk * Impact_of_Risk

Conservative Forecast = Base Forecast * K_total
K_total = `K_trial_intent` * `K_activation_during_trial` * `K_payment_success` * `K_first_renewal_quality`
```
...
|---|---:|---|
| `K_trial_intent` | 0.60-0.95 | понижение, если trial starts стали менее осознанными |
| `K_activation_during_trial` | 0.65-0.95 | понижение, если пользователи не дошли до value moment |
| `K_payment_success` | 0.70-0.98 | понижение за failed payments и card issues |
| `K_first_renewal_quality` | 0.60-0.95 | понижение, если первая оплата не подтверждается renewal |
...
- Фиксировать trial_start, trial_cancel, trial_end, billing_attempt, payment_success.
- Разделять no-card trial, card-required trial, paid trial и freemium upgrade.
- Хранить activation_during_trial и paywall_exposure.
- Сохранять first_renewal_status, refund и chargeback.
