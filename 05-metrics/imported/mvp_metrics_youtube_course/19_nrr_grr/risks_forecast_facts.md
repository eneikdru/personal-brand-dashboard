# 19. Risks, Forecast and Facts: NRR and GRR

## Слабые стороны с точки зрения аналитической философии
- NRR может скрыть высокий logo churn, если крупные клиенты расширяются.
- GRR честнее про защиту базы, но не показывает expansion potential.
- Метрики могут выглядеть здоровыми в enterprise-сегменте и одновременно скрывать слабость SMB.
- NRR не объясняет, вызвано ли расширение продуктовой ценностью, sales pressure или изменением цены.

## Где метрика может обмануть
Метрика обманывает, когда ее начинают воспринимать как реальность, а не как модель реальности. Для `NRR and GRR` главный риск - забыть, что показатель зависит от определения события, выбранного знаменателя, периода, сегмента и качества данных.
...
```text
Forecast NRR = (starting_MRR + forecast_expansion - forecast_contraction - forecast_churn) / starting_MRR

Risk Coefficient = 1 - Probability_of_Risk * Impact_of_Risk

Conservative Forecast = Base Forecast * K_total
K_total = `K_expansion_reality` * `K_churn_visibility` * `K_customer_mix` * `K_price_change_effect`
```
...
|---|---:|---|
| `K_expansion_reality` | 0.60-0.95 | понижение, если expansion не подтвержден usage или pipeline |
| `K_churn_visibility` | 0.65-0.95 | понижение, если churn risk плохо виден |
| `K_customer_mix` | 0.70-0.95 | понижение, если меняется структура клиентов |
| `K_price_change_effect` | 0.70-0.95 | понижение, если рост вызван ценой, а не ценностью |
...
- Фиксировать starting cohort existing customers без new customers.
- Хранить expansion, contraction и churn movements отдельно.
- Записывать reason: seat expansion, usage growth, price increase, downgrade, cancel.
- Сохранять account health, usage depth и renewal date.
