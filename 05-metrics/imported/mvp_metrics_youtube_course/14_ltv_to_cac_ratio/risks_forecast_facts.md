# 14. Risks, Forecast and Facts: LTV:CAC Ratio

## Слабые стороны с точки зрения аналитической философии
- Ratio красиво сжимает экономику в одну цифру и прячет cash-flow проблему.
- Высокий LTV:CAC может быть результатом завышенного LTV или неполного CAC.
- Средний ratio скрывает убыточные каналы и сегменты.
- Метрика не показывает скорость окупаемости и ограничение объема канала.

## Где метрика может обмануть
Метрика обманывает, когда ее начинают воспринимать как реальность, а не как модель реальности. Для `LTV:CAC Ratio` главный риск - забыть, что показатель зависит от определения события, выбранного знаменателя, периода, сегмента и качества данных.
...
```text
Forecast LTV:CAC = forecast_gross_profit_LTV / forecast_fully_loaded_CAC

Risk Coefficient = 1 - Probability_of_Risk * Impact_of_Risk

Conservative Forecast = Base Forecast * K_total
K_total = `K_ltv_confidence` * `K_cac_completeness` * `K_segment_alignment` * `K_payback_risk`
```
...
|---|---:|---|
| `K_ltv_confidence` | 0.50-0.95 | понижение за неопределенность LTV |
| `K_cac_completeness` | 0.65-0.98 | понижение за неполный учет acquisition costs |
| `K_segment_alignment` | 0.60-0.95 | понижение, если LTV и CAC взяты из разных сегментов |
| `K_payback_risk` | 0.60-0.95 | понижение, если окупаемость слишком длинная |
...
- Фиксировать LTV_source и CAC_source для каждого ratio.
- Хранить segment_id: канал, тариф, страна, sales motion.
- Сохранять gross margin assumptions и acquisition cost inclusions.
- Рядом фиксировать payback period, чтобы ratio не жил отдельно.
