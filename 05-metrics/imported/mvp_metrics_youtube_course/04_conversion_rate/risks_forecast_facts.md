# 04. Risks, Forecast and Facts: Conversion Rate

## Слабые стороны с точки зрения аналитической философии
- Conversion rate особенно легко обманывает неправильным знаменателем.
- Метрика может улучшиться за счет отсечения сложных пользователей, а не за счет улучшения продукта.
- Конверсия не показывает качество дальнейшего поведения после целевого действия.
- Процент без абсолютных объемов и доверительного интервала создает ложную уверенность.

## Где метрика может обмануть
Метрика обманывает, когда ее начинают воспринимать как реальность, а не как модель реальности. Для `Conversion Rate` главный риск - забыть, что показатель зависит от определения события, выбранного знаменателя, периода, сегмента и качества данных.
...
```text
Forecast Conversions = eligible_users_or_sessions * expected_conversion_rate

Risk Coefficient = 1 - Probability_of_Risk * Impact_of_Risk

Conservative Forecast = Base Forecast * K_total
K_total = `K_denominator_match` * `K_attribution` * `K_statistical_confidence` * `K_downstream_quality`
```
...
|---|---:|---|
| `K_denominator_match` | 0.70-1.00 | понижение, если будущий eligible pool отличается от исторического |
| `K_attribution` | 0.75-0.95 | понижение за неопределенность источника конверсии |
| `K_statistical_confidence` | 0.60-1.00 | понижение при малой выборке или широком confidence interval |
| `K_downstream_quality` | 0.70-0.95 | понижение, если конверсия не подтверждена retention/revenue |
...
- Фиксировать start_event, target_event и conversion_window.
- Разделять user-level, session-level и attempt-level факты.
- Хранить все попытки, но отдельно считать уникальных пользователей.
- Логировать failed conversions: ошибки, abandonment, timeout, payment decline.
