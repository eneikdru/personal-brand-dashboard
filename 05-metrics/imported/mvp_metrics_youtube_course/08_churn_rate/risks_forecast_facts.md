# 08. Risks, Forecast and Facts: Churn Rate

## Слабые стороны с точки зрения аналитической философии
- Churn часто воспринимают как единое событие ухода, хотя причины ухода разные и требуют разных решений.
- Метрика может скрывать различие между voluntary и involuntary churn.
- Customer churn и revenue churn отвечают на разные вопросы, но их часто смешивают.
- Высокий churn новых пользователей и churn зрелых клиентов имеют разную философию проблемы.

## Где метрика может обмануть
Метрика обманывает, когда ее начинают воспринимать как реальность, а не как модель реальности. Для `Churn Rate` главный риск - забыть, что показатель зависит от определения события, выбранного знаменателя, периода, сегмента и качества данных.
...
```text
Forecast Lost Customers = customers_at_start * expected_churn_rate

Risk Coefficient = 1 - Probability_of_Risk * Impact_of_Risk

Conservative Forecast = Base Forecast * K_total
K_total = `K_reason_visibility` * `K_subscription_age_mix` * `K_payment_recovery` * `K_plan_segment`
```
...
|---|---:|---|
| `K_reason_visibility` | 0.70-0.95 | понижение, если причины churn плохо классифицированы |
| `K_subscription_age_mix` | 0.65-0.95 | понижение, если меняется доля новых/зрелых клиентов |
| `K_payment_recovery` | 0.75-0.98 | понижение или повышение осторожности при failed payments и dunning |
| `K_plan_segment` | 0.70-0.95 | понижение при переносе churn между тарифами или рынками |
...
- Фиксировать cancel_requested_at, access_ended_at и billing_failed_at отдельно.
- Записывать churn_type: voluntary, involuntary, downgrade, pause, refund, chargeback.
- Хранить subscription_age и billing_cycle_number.
- Фиксировать save_attempt, winback, dunning outcome и final status.
