# 06. Risks, Forecast and Facts: Retention Rate

## Слабые стороны с точки зрения аналитической философии
- Retention может наказывать продукты с редкой, но высокой ценностью.
- Неправильный active event превращает retention в счетчик открытий, а не возвращения к ценности.
- Classic, rolling и bracket retention дают разные картины, и их легко подменить друг другом.
- Средний retention скрывает сильные различия каналов, стран, тарифов и use cases.

## Где метрика может обмануть
Метрика обманывает, когда ее начинают воспринимать как реальность, а не как модель реальности. Для `Retention Rate` главный риск - забыть, что показатель зависит от определения события, выбранного знаменателя, периода, сегмента и качества данных.
...
```text
Forecast Retained Users at N = cohort_size * expected_retention_N

Risk Coefficient = 1 - Probability_of_Risk * Impact_of_Risk

Conservative Forecast = Base Forecast * K_total
K_total = `K_activity_meaning` * `K_cohort_maturity` * `K_frequency_fit` * `K_seasonality`
```
...
|---|---:|---|
| `K_activity_meaning` | 0.70-0.95 | понижение, если active event слабо связан с ценностью |
| `K_cohort_maturity` | 0.65-1.00 | понижение для свежих когорт с right censoring |
| `K_frequency_fit` | 0.75-1.00 | понижение, если окно retention не соответствует природе продукта |
| `K_seasonality` | 0.70-0.98 | понижение за сезонные и календарные эффекты |
...
- Фиксировать cohort_start_event и active_event.
- Хранить cohort_size неизменным для выбранного расчета.
- Записывать period_index: D1, D7, W4, M3 и т.д.
- Отдельно фиксировать resurrected activity, чтобы не смешивать ее с удержанием новой когорты.
