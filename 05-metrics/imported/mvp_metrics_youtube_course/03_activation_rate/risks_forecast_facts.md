# 03. Risks, Forecast and Facts: Activation Rate

## Слабые стороны с точки зрения аналитической философии
- Activation может быть красивой метрикой 'первого клика', если событие не связано с будущей ценностью.
- Метрика часто завышается, когда команда облегчает действие, но не увеличивает реальный успех пользователя.
- Один activation event может не подходить разным сегментам и use cases.
- Activation показывает ранний сигнал, но не доказывает долгосрочный retention или willingness to pay.

## Где метрика может обмануть
Метрика обманывает, когда ее начинают воспринимать как реальность, а не как модель реальности. Для `Activation Rate` главный риск - забыть, что показатель зависит от определения события, выбранного знаменателя, периода, сегмента и качества данных.
...
```text
Forecast Activated Users = new_users * expected_activation_rate

Risk Coefficient = 1 - Probability_of_Risk * Impact_of_Risk

Conservative Forecast = Base Forecast * K_total
K_total = `K_aha_validity` * `K_onboarding_execution` * `K_user_intent` * `K_time_window`
```
...
|---|---:|---|
| `K_aha_validity` | 0.65-0.95 | понижение, если aha moment не доказан связью с retention/payment |
| `K_onboarding_execution` | 0.75-0.98 | понижение за риски UX, багов и неполного rollout |
| `K_user_intent` | 0.70-0.95 | понижение, если новый трафик менее мотивирован |
| `K_time_window` | 0.80-1.00 | понижение, если окно прогноза отличается от окна исторического расчета |
...
- Логировать signup/start event и activation event как отдельные факты.
- Фиксировать activation_window: D0, D1, D7 или другой выбранный интервал.
- Хранить eligibility: мог ли пользователь физически активироваться.
- Сохранять onboarding_version, experiment_id и причину исключения из знаменателя.
